        ...
        <div class="section-body">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('admin_photo_edit_submit',$photo->id) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group mb-3">
                                    <label>Existing Photo</label>
                                    <div>
                                        <img src="{{ asset('uploads/'.$photo->photo) }}" class="w_200">
                                    </div>
                                </div>
                                <div class="form-group mb-3">
                                    ...
                                    <select name="photo_category_id" class="form-select" >
                                        @foreach($photo_categories as $photo_category)
                                            <option value="{{ $photo_category->id }}" 
                                                @if($photo_category->id==$photo->photo_category_id)
                                                    selected
                                                @endif>
                                                {{ $photo_category->name }}
                                            </option>
